...
function Snackrificer:ComputeValue(item)
    ...
    local craving = TheWorld.components.thegnaw:GetCurrentCraving()
    local cravingbonus = item.components.snackrificable:SatisfiesCraving(craving) and CRAVING_BONUS or 0
    local saltbonus = 0
    ...
    if rewardTable then
        ...
        if reward then
            ...
            if satisfaction < 0 then
                self:SubtractCravingViolation(reward)
            ...
            local noveltymultiplier = TheWorld.components.thegnaw:GetNoveltyScore(item.prefab)
            for k, v in pairs(reward) do
                local vv = math.floor(noveltymultiplier * v)
                reward[k] = vv
            end
            self.reward = {}
            for coinprefab, amount in pairs(reward) do
                for i=1,amount do
                    local coin = SpawnPrefab(coinprefab)
...
function Snackrificer:Snackrifice(doer, item)
    ...
    if value > 0 then
        ...
        self.inst:DoTaskInTime(3, function() TheWorld.components.thegnaw:ChangeSatisfaction(doer, satisfaction) end)
    end
    ...
    -- Change the craving
    TheWorld.components.thegnaw:PickCraving()
    TheWorld.components.thegnaw:RememberMeal(item.prefab)
end
