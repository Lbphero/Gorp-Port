...
local growth_stages =
{
    {
        name = "short",
        time = function(inst) return GetRandomWithVariance(TUNING.DECIDUOUS_GROW_TIME[1].base,
        TUNING.DECIDUOUS_GROW_TIME[1].random) end,
        fn = SetShort,
        growfn = GrowShort
    },
    {
        name = "normal",
        time = function(inst) return GetRandomWithVariance(TUNING.DECIDUOUS_GROW_TIME[2].base,
        TUNING.DECIDUOUS_GROW_TIME[2].random) end,
        fn = SetNormal,
        growfn = GrowNormal
    },
    {
        name = "tall",
        time = function(inst) return GetRandomWithVariance(TUNING.DECIDUOUS_GROW_TIME[3].base,
        TUNING.DECIDUOUS_GROW_TIME[3].random) end,
        fn = SetTall,
        growfn = GrowTall
    },
...
local function ChopTree(inst, chopper, chops)
    ...
    if inst.components.tappable:IsTapped() then
        inst.components.tappable:UninstallTap(chopper, true)
...
local function ChopDownTree(inst, chopper)
    ...
    if he_right then
        ...
        if inst.components.growable ~= nil and inst.components.growable.stage == 3 then
            inst.components.lootdropper:SpawnLootPrefab("saptree_nut", pt - TheCamera:GetRightVec())
        end
...
local function fn(tree_def)
    ...
    -- Maybe we can make it a monster someday.
    inst.monster = false
